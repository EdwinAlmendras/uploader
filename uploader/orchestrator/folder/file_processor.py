...
class FileProcessor:
    ...
    async def process(self, file_path: Path, rel_path: Path, dest_path: str, progress_callback=None) -> Tuple[UploadResult, Optional[float], Optional[str]]:
        ...
        _log_memory("FileProcessor.process started", f"{file_path.name} ({file_size_mb:.2f} MB)")
        try:
            ...
            _log_memory("Before upload_video call", file_path.name)
            try:
                handle = await self._storage.upload_video(file_path, mega_dest, source_id, progress_callback)
                _log_memory("After upload_video call", file_path.name)
                if not handle:
            ...
            # Analyze and save metadata (replace source_id generated by analysis with our pre-generated one)
            logger.info("Analyzing %s...", file_path.name)
            _log_memory("Before video analysis", file_path.name)
            if is_video(file_path):
                tech_data = await self._analyzer.analyze_video_async(file_path)
                _log_memory("After video analysis", file_path.name)
                ...
                _log_memory("FileProcessor.process completed", file_path.name)
