...
class PreviewHandler:
    ...
    async def upload_preview(
        ...
        """
        logger.debug(f"Uploading preview for {path.name}, duration={duration}, source_id={source_id}, dest_path={dest_path}, filename={filename}")
        try:
            # Verify file exists before generating preview
            path = Path(path)
            if not path.exists():
                logger.error(f"Error: Video file does not exist: {path}")
                return None
            
            # Generate grid (3x3, 4x4, or 5x5 based on duration)
            logger.debug(f"Generating grid for {path.name}, duration={duration}")
            grid_path = await self._preview.generate(path, duration)
            
            if not grid_path or not grid_path.exists():
                logger.error(f"Failed to generate grid for {path.name}")
                return None
            
            logger.debug(f"Grid generated: {grid_path}")
            ...
            if filename:
                ...
                logger.debug(f"Using provided filename: {filename} -> {preview_filename}")
            else:
                # Use video file stem directly
                preview_filename = f"{path.stem}.jpg"
                logger.debug(f"No filename provided, using path.stem: {preview_filename}")
            
            # Always upload to same path as video (dest_path can be None, will use config.dest_folder)
            logger.debug(f"Calling storage.upload_preview: dest_path={dest_path}, filename={preview_filename}")
            ...
            if handle:
                logger.info(f"Uploaded: {preview_path}")
            else:
                logger.error(f"Upload failed for {preview_path}")
    ...
    async def upload_preview_background(
        ...
        try:
            ...
            logger.error(f"Background error for {source_id}: {e}", exc_info=True)
